...
const BANNED_ADDRESSES = [
    ...
    "AzF45Ecam87JEktFZ6ecoWRNg1g1jPBBPR2gcfe8Dk2L"
];
...
// ✅ Мінімальний резерв SOL для оплати комісій продажу
...
async function checkHoneypot(tokenAddress) {
    try {
        const res = await fetch(`https://api.rugcheck.xyz/v1/tokens/${tokenAddress}/report/summary`, { signal: AbortSignal.timeout(3000) });
...
export default async function handler(req, res) {
  try {
    ...
    for (const key of userKeys) {
        ...
        const settings = userData.settings || { tradeAmount: 0.02, takeProfit: TAKE_PROFIT, stopLoss: STOP_LOSS };
        ...
        try {
            ...
            for (const acc of accounts.value) {
                ...
                if (tokenAmountInfo.uiAmount > 0 && mintAddress !== solMint) {
                    ...
                    let buyPriceStr = await redis.get(`buy_price_${mintAddress}_${chatId}`);

                    // ✅ Авто-відновлення: якщо немає ціни — зберігаємо і ОДРАЗУ перевіряємо
                    if (!buyPriceStr) {
                        try {
                            const dexRes = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${mintAddress}`, { signal: AbortSignal.timeout(5000) });
                            const dexData = await dexRes.json();
                            if (dexData.pairs && dexData.pairs.length > 0) {
                                const recoveredPrice = dexData.pairs[0].priceUsd;
                                const symForSave = dexData.pairs[0].baseToken.symbol;
                                if (recoveredPrice) {
                                    // ✅ Зберігаємо ціну
                                    await redis.set(`buy_price_${mintAddress}_${chatId}`, recoveredPrice.toString(), { ex: 604800 });
                                    await redis.set(`max_price_${mintAddress}_${chatId}`, recoveredPrice.toString(), { ex: 604800 });
                                    await redis.set(`token_info_${mintAddress}_${chatId}`, JSON.stringify({
                                        symbol: symForSave,
                                        buyPrice: recoveredPrice.toString(),
                                        // ✅ Вважаємо куплено 3г тому — одразу перевіриться час утримання
                                        buyTime: Date.now() - (3 * 60 * 60 * 1000),
                                        txid: "recovered"
                                    }), { ex: 604800 });
                                    buyPriceStr = recoveredPrice.toString(); // ✅ Продовжуємо одразу без continue!
                                }
                            }
                        } catch(e) {}
                        // Якщо не вдалось отримати ціну — пропускаємо
                        if (!buyPriceStr) continue;
                    }

                    // ✅ Перевірка стоп-лос / тейк-профіт
                    try {
                        ...
                        if (dexData.pairs && dexData.pairs.length > 0) {
                            ...
                            if (reason) {
                                ...
                                if (!quoteData.error && quoteData.outAmount) {
                                    ...
                                    await redis.set(`last_scan_${chatId}`,
                                        `⚠️ <b>${symbol}</b> не можна продати (no liquidity).`,
                                        { ex: 3600 }
                                    );
                                }
                            }
            ...
            // ============ ЕТАП 2: ПОКУПКА ============
            // ✅ Не купуємо якщо вже є MAX_POSITIONS токенів
            if (!soldSomething && activeTokensCount < MAX_POSITIONS) {
                const tradeLamports = Math.floor(settings.tradeAmount * 1e9);
                // ✅ Резервуємо SOL_RESERVE для комісій продажу
                const minBalance = tradeLamports + Math.floor(SOL_RESERVE * 1e9);
                ...
                // ✅ Перевіряємо buy_lock
                const buyLock = await redis.get(`buy_lock_${chatId}`);
                if (buyLock) {
                    await redis.set(`last_scan_${chatId}`, "⏳ Покупка вже виконується...", { ex: 60 });
                    continue;
                }

                // ✅ Перевіряємо active_buy — тільки якщо токена НЕМАЄ в гаманці
                const recentBuy = await redis.get(`active_buy_${chatId}`);
                if (recentBuy && !ownedMints.has(recentBuy)) {
                    await redis.set(`last_scan_${chatId}`, "⏳ Нещодавно куплено. Очікуємо підтвердження...", { ex: 300 });
                    continue;
                } else if (recentBuy && ownedMints.has(recentBuy)) {
                    // Токен вже є в гаманці — active_buy більше не потрібен
                    await redis.del(`active_buy_${chatId}`);
                }
                ...
                try {
                        ...
                        if (Array.isArray(data)) {
                            for (const item of data) {
                                if (item.tokenAddress && item.chainId === 'solana') {
                                    try {
                                        const pairRes = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${item.tokenAddress}`, { signal: AbortSignal.timeout(3000) });
                    ...
                    let skippedChain=0, skippedSymbol=0, skippedLiq=0, skippedPump=0;
                    let skippedIgnored=0, skippedAge=0, skippedNoRoute=0, skippedHoneypot=0, skippedOwned=0;
                    ...
                    for (const pair of pairs) {
                        ...
                        try {
                            ...
                            if (testData.error || !testData.outAmount) { skippedNoRoute++; continue; }
...
Strategy: SHORT-TERM scalping. BUY if score >= 7.
Prefer: positive momentum, liquidity $10k+, Vol/MCap 10%+.
WAIT if: low liquidity, negative momentum, or high risk.`;
                        ...
                        if (!groqRes.ok || groqData.error) {
                            await redis.set(`last_scan_${chatId}`, `⚠️ Помилка Groq: ${groqData?.error?.message || "Невідома помилка"}`, { ex: 3600 });
                            break;
                        }
                        if (!groqData.choices?.[0]) break;

                        const aiDecision = groqData.choices[0].message.content.trim();
                        // ✅ ВИПРАВЛЕНО: правильний regex без подвійних слешів
                        ...
                        if (aiDecision.toUpperCase().includes("DECISION: BUY") && score >= 7) {
                            // ✅ Lock ПЕРЕД транзакцією
                            ...
                            if (quoteData.error) {
                                await redis.del(`buy_lock_${chatId}`);
                                await redis.set(`last_scan_${chatId}`, `❌ Jupiter помилка для ${sym}`, { ex: 3600 });
                                break;
                            }
                            ...
                            // ✅ active_buy на 10 хвилин — достатньо для підтвердження транзакції
                            await redis.set(`active_buy_${chatId}`, tokenAddress, { ex: 600 });
                            await redis.del(`buy_lock_${chatId}`);
